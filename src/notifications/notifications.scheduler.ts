...
export class NotificationsScheduler implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  private async runAll(trigger: string) {
    try {
      // Pass undefined = no branch filter = scan ALL branches
      ...
      this.logger.log(
        `[${trigger}] Alerts generated — lowStock: ${lowStock.created}, expiry: ${expiry.created}, paymentDue: ${paymentDue.created}`,
      );
    } catch (err) {
