...
function monthsAgo(n) { return daysAgo(n * 30); }
...
async function main() {
    ...
    const [sup1, sup2, sup3] = await Promise.all([
        prisma.supplier.create({ data: { id: 'SUP-001', name: 'Global Pharma Distributors', contactPerson: 'Mr. Sharma', phone: '9876543211', email: 'sales@globalpharma.com', address: 'Industrial Area, Coimbatore', gstin: '29ABCDE1234F1Z5', drugLicense: 'DL-12345', paymentTerms: client_1.PaymentTerms.NET_30, branchId: hq.id } }),
        prisma.supplier.create({ data: { id: 'SUP-002', name: 'Zydus Healthcare', contactPerson: 'Ms. Gupta', phone: '9876543212', email: 'zydus@example.com', address: 'Ahmedabad', gstin: '24ABCDE1234F1Z5', drugLicense: 'DL-23456', paymentTerms: client_1.PaymentTerms.NET_45, branchId: hq.id } }),
        prisma.supplier.create({ data: { id: 'SUP-003', name: 'Sun Pharma Distribution', contactPerson: 'Mr. Verma', phone: '9876543213', email: 'sunpharma@example.com', address: 'Mumbai', gstin: '27ABCDE1234F1Z5', drugLicense: 'DL-34567', paymentTerms: client_1.PaymentTerms.NET_30, branchId: hq.id } }),
    ]);
    const [sup4, sup5] = await Promise.all([
        prisma.supplier.create({ data: { id: 'SUP-004', name: 'Natco Fine Chemicals', contactPerson: 'Mr. Rao', phone: '9876543214', email: 'natco@example.com', address: 'Hyderabad', gstin: '36ABCDE1234F1Z5', drugLicense: 'DL-45678', paymentTerms: client_1.PaymentTerms.NET_60, branchId: br1.id } }),
        prisma.supplier.create({ data: { id: 'SUP-005', name: 'Hetero Logistics', contactPerson: 'Ms. Rao', phone: '9876543215', email: 'hetero@example.com', address: 'Hyderabad', gstin: '36ABCDE5678F1Z5', drugLicense: 'DL-56789', paymentTerms: client_1.PaymentTerms.NET_30, branchId: br1.id } }),
    ]);
    ...
    const [cust1, cust3, cust4, walkIn] = await Promise.all([
        prisma.customer.create({ data: { id: 'CUS-001', name: 'Apollo Hospital - Madurai', phone: '9944112233', email: 'pharmacy@apollomadurai.com', address: '11, KK Nagar, Madurai - 625020', type: client_1.CustomerType.HOSPITAL, creditLimit: 500000, currentOutstanding: 0, gstin: '33AABCA1234A1Z5', dlNumber: 'TN/MDU/20B/2021/1001', branchId: hq.id } }),
        ...
        prisma.customer.create({ data: { id: 'CUS-004', name: 'Murugan S', phone: '9944667788', address: '12/3, Simmakkal, Madurai', type: client_1.CustomerType.REGULAR, creditLimit: 10000, currentOutstanding: 0, loyaltyPoints: 320, branchId: hq.id } }),
        prisma.customer.create({ data: { id: 'CUS-WALKIN-HQ', name: 'Walk-in Customer (HQ)', phone: '0000000001', type: client_1.CustomerType.WALK_IN, creditLimit: 0, currentOutstanding: 0, branchId: hq.id } }),
    ]);
    const [cust2, cust5, walkIn2] = await Promise.all([
        prisma.customer.create({ data: { id: 'CUS-002', name: 'MIOT Hospital - Chennai', phone: '9944223344', email: 'purchase@miothospitals.com', address: '4/112, Mount Poonamallee Road, Chennai - 600089', type: client_1.CustomerType.HOSPITAL, creditLimit: 750000, currentOutstanding: 0, gstin: '33AABCM5678B2Z1', dlNumber: 'TN/CHE/20B/2020/0852', branchId: br1.id } }),
        prisma.customer.create({ data: { id: 'CUS-005', name: 'Lakshmi K', phone: '9944778899', address: '56, Tallakulam, Madurai', type: client_1.CustomerType.REGULAR, creditLimit: 15000, currentOutstanding: 0, loyaltyPoints: 540, branchId: br1.id } }),
        prisma.customer.create({ data: { id: 'CUS-WALKIN-BR1', name: 'Walk-in Customer (Chennai)', phone: '0000000002', type: client_1.CustomerType.WALK_IN, creditLimit: 0, currentOutstanding: 0, branchId: br1.id } }),
    ]);
    ...
    const hqPO = await prisma.purchaseOrder.create({
        data: {
            ...
            totalAmount: 185000,
            status: client_1.POStatus.FULLY_RECEIVED,
    ...
    const hqGRN = await prisma.gRN.create({
        data: {
            ...
            totalAmount: 185000,
            status: client_1.GRNStatus.VERIFIED,
            items: {
                create: [
    ...
    const hqInvoices = [];
    const hqInvoiceData = [
        { days: 90, amount: 8500, cust: cust1, mode: client_1.PaymentMode.CREDIT, status: client_1.InvoiceStatus.CREDIT, amountPaid: 0, product: products[0], batch: hqBatch1, qty: 20 },
        { days: 75, amount: 12400, cust: cust1, mode: client_1.PaymentMode.CASH, status: client_1.InvoiceStatus.PAID, amountPaid: 12400, product: products[4], batch: hqBatch3, qty: 50 },
        { days: 60, amount: 63000, cust: cust1, mode: client_1.PaymentMode.CREDIT, status: client_1.InvoiceStatus.CREDIT, amountPaid: 0, product: products[3], batch: hqBatch2, qty: 30 },
    ...
    for (let i = 0; i < hqInvoiceData.length; i++) {
        ...
        const inv = await prisma.invoice.create({
            data: {
                ...
                billingType: d.cust.type === client_1.CustomerType.WHOLESALE ? client_1.BillingType.WHOLESALE : client_1.BillingType.RETAIL,
                branchId: hq.id,
    ...
    if (firstHqInv) {
        await prisma.creditNote.create({
            data: {
                ...
                sgst: 25.5,
                totalAmount: 476,
                settlementMode: client_1.SettlementMode.REFUND,
                createdById: hqPharmacist.id,
                items: {
                    create: [{
    ...
    await prisma.purchaseReturn.create({
        data: {
            ...
            sgst: 31.2,
            totalAmount: 582.4,
            status: client_1.PurchaseReturnStatus.ACCEPTED,
    ...
    const br1InvoiceData = [
        { days: 80, amount: 94500, cust: cust2, mode: client_1.PaymentMode.CREDIT, status: client_1.InvoiceStatus.CREDIT, amountPaid: 0, product: products[9], batch: br1Batch3, qty: 4 },
        { days: 60, amount: 42000, cust: cust2, mode: client_1.PaymentMode.CREDIT, status: client_1.InvoiceStatus.PARTIAL, amountPaid: 20000, product: products[8], batch: br1Batch2, qty: 5 },
        { days: 45, amount: 22500, cust: cust2, mode: client_1.PaymentMode.CASH, status: client_1.InvoiceStatus.PAID, amountPaid: 22500, product: products[6], batch: br1Batch1, qty: 50 },
    ...
    if (firstBr1Inv) {
        await prisma.creditNote.create({
            data: {
                ...
                sgst: 1344,
                totalAmount: 25088,
                settlementMode: client_1.SettlementMode.CREDIT,
                createdById: br1Pharmacist.id,
                items: {
                    create: [{
    ...
    await prisma.purchaseReturn.create({
        data: {
            ...
            sgst: 696,
            totalAmount: 12992,
            status: client_1.PurchaseReturnStatus.SETTLED,
    ...
    await Promise.all(Array.from(outstandingMap.entries()).map(([customerId, amount]) => prisma.customer.update({ where: { id: customerId }, data: { currentOutstanding: amount } })));
