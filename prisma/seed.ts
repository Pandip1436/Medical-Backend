...
async function main() {
  ...
  const [cust1, cust3, cust4] = await Promise.all([
    ...
    prisma.customer.create({ data: { id: 'CUS-004', name: 'Murugan S', phone: '9944667788', address: '12/3, Simmakkal, Madurai', type: CustomerType.RETAIL, creditLimit: 10000, currentOutstanding: 0, branchId: hq.id } }),
  ]);
  // Chennai Customers
  const [cust2, cust5] = await Promise.all([
    ...
    prisma.customer.create({ data: { id: 'CUS-005', name: 'Lakshmi K', phone: '9944778899', address: '56, Tallakulam, Madurai', type: CustomerType.RETAIL, creditLimit: 15000, currentOutstanding: 0, branchId: br1.id } }),
  ]);
